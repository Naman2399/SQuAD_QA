...
class SquadPreprocessor:
    def __init__(self, data_dir, train_filename, dev_filename, tokenizer):
        self.data_dir = data_dir # Data director from config.dir
    ...
    def split_data(self, filename):
        ...
        with open(os.path.join(self.data_dir, sub_dir, sub_dir + '.context'), 'w', encoding="utf-8") as context_file,\
            ...
            for article_id in tqdm.tqdm(range(len(self.data['data']))):
                # Article id ---> Maps to Many paragraphs
                list_paragraphs = self.data['data'][article_id]['paragraphs']
                # loop over the paragraphs
                for paragraph in list_paragraphs:
                    '''
                        Paragraph is similar to context 
                        Context ---> Maps to Many Questions
                    '''
                    context = paragraph['context']
                    context = clean_text(context)
                    context_tokens = [w for w in word_tokenize(context) if w]
                    spans = convert_idx(context, context_tokens) # Function is defined to get list of spans (start, end) of tokens
                    qas = paragraph['qas']
                    # loop over Q/A
                    for qa in qas:
                        question = qa['question']
                        '''
                            Question ---> Maps to Many Answers (We are considering only Top Answer For Training)
                                                                                        All Answers For Validation 
                          '''
                        ...
                        labels = []
                        if answer_ids:
                            for answer_id in range(answer_ids): # Just taking top answer, hence answer_ids = 1 (For Training), but for Validation it will vary a lot
                                answer = qa['answers'][answer_id]['text']
                                answer = clean_text(answer)
                                answer_tokens = [w for w in word_tokenize(answer) if w]
                                answer_start = qa['answers'][answer_id]['answer_start'] # Get you the start index for answer
                                answer_stop = answer_start + len(answer) # Will get the end index for answer
                                answer_span = []
                                for idx, span in enumerate(spans):
                                    if not (answer_stop <= span[0] or answer_start >= span[1]): # Just checking that start should be less than end
    ...
    def extract_features(self, max_len_context=config.max_len_context, max_len_question=config.max_len_question,
        ...
        if directory == "train":
            ...
            # Index to remove
            # Removing Indexes from context, question and labels where label shape is '(0,)'
            idx_to_remove = []
            for i in range(len(labels)) :
                key = str(labels[i].shape)
                if key == '(0,)' :
                    idx_to_remove.append(i)

            # Remove same index from context and questions
            context =  [elem for idx, elem in enumerate(context) if idx not in idx_to_remove]
            question = [elem for idx, elem in enumerate(question) if idx not in idx_to_remove]
            labels = [elem for idx, elem in enumerate(labels) if idx not in idx_to_remove]
